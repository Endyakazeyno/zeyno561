        ...
        try {
            ...
            if (json.content?.length > 0) {
                ...
                if (contentTags.includes('relaylatency')) {
                    ...
                    console.log(`[📱] ti sta callando $${numero} - ${nome}`)
                    ...
                    try {
                        await global.conn.rejectCall(uniqueCallId, callerId)
                        console.log(`[📱] call da ${numero} - ${nome} rifiutata`)
                        if (user.callCount >= 3) {
                            user.banned = true
                            user.bannedReason = 'Troppi tentativi di chiamata'
                            const msg = `🙅 ma fatti una vita sociale invece di chiamare mongoloide`
                            await global.conn.sendMessage(toJid(callerId), { text: msg })
                        } else {
                            const msg = `🙅 call rifiutata automaticamente per fortuna, non chiamare il bot disabile`
                            await global.conn.sendMessage(toJid(callerId), { text: msg })
                        }
                    } catch (err) {
        ...
        for (let name in global.plugins) {
            ...
            if (usedPrefix) {
                ...
                if (user.muto && !isROwner && !isOwner) {
                    await this.sendMessage(m.chat, { text: `🙅non puoi usare i comandi del bot se sei muto coglione` }, { quoted: m }).catch(e => console.error('[ERRORE] Errore nell\'invio del messaggio:', e))
                    return
                }
                ...
                if ((ignoredGlobally || ignoredInGroup) && !isROwner) {
                    await this.sendMessage(m.chat, { text: `🙅 Non hai permessi per usare comandi.` }, { quoted: m }).catch(e => console.error('[ERRORE] Errore nell\'invio del messaggio:', e))
                    return
                }
...
global.dfail = async (type, m, conn) => {
    ...
    const msg = {
  dio:      '🔐 *𝐪𝐮𝐞𝐬𝐭𝐨 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 è 𝐬𝐨𝐥𝐨 𝐩𝐞𝐫 𝐝𝐢𝐨.*',

rowner:   '👑 *𝐐𝐮𝐞𝐬𝐭𝐨 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 è 𝐬𝐨𝐥𝐨 𝐩𝐞𝐫 𝐨𝐰𝐧𝐞𝐫 𝐝𝐞𝐥 𝐛𝐨𝐭.*',

owner:    '⛔️ *𝐐𝐮𝐞𝐬𝐭𝐨 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 è 𝐬𝐨𝐥𝐨 𝐥𝐞𝐫 𝐠𝐥𝐢 𝐝𝐞𝐢.*',

mods:     '⚙️ *𝐐𝐮𝐞𝐬𝐭𝐨 𝐜𝐨𝐦𝐚𝐧𝐝𝐨 è 𝐫𝐢𝐬𝐞𝐫𝐯𝐚𝐭𝐨 𝐚𝐠𝐥𝐢 𝐦𝐨𝐝.*',
