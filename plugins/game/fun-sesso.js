...
let handler = async (m, { conn }) => {
  try {
    ...
    if (!target) {
      const q = buildContextMsg('*sesso*')
      await conn.sendMessage(chat, {
        text: `${boldUnicode('*⚠️ Devi menzionare qualcuno o rispondere a un messaggio!*')}\n\n${boldUnicode('Esempio:')}\n\n${boldUnicode('.sesso @utente')}`
      }, { quoted: q })
      return
    }

    const qAnim = buildContextMsg('*sesso*')
    const seq = [
      boldUnicode('🤟🏻 Ora faccio sesso con') + tag(target) + boldUnicode('...'),
      boldUnicode('🤟🏻🤤 Le sto sfondando la figa...'),
      boldUnicode('🤟🏻🥵 Sento come gode la troia...'),
      boldUnicode('"😍 Ohhsy continua, ahh, ahh, ora fammi un pompino"'),
      boldUnicode('"😍 suchiarmelo tutta troia"'),
      boldUnicode('🤟🏻🤤 sto per sborrare nella tua benedetta bocca ahhh'),
      boldUnicode('🤟🏻🤤 lo stronzo sta per sborrare sulla sua figa, attento a non metterla incinta...'),
      boldUnicode('"😍😍 AHHH!!, AHHH!!"'),
      boldUnicode('😎 La troia li è piaciuto'),
      boldUnicode('💦 alla troia piace la sua sborra')
    ]
    ...
    await conn.sendMessage(
      ...
      { quoted: buildContextMsg('*sesso*') }
    )

  } catch (e) {
    console.error('sesso error:', e)
  }
}

handler.help = ['sesso @utente (o reply)']
handler.tags = ['fun']
handler.command = ['sesso', 'sex']
handler.group = true
