...
(async () => {
    try {
        ...
        console.log(chalk.hex('#3F00FF').bold(`𝚭𝚵𝚼𝚴𝚰 𝚩𝚰𝚮 connesso correttamente`));
    } catch (error) {
        console.error(chalk.bold.bgHex('#3F00FF')(`🥀 Errore nell'avvio del bot: ${error?.stack || error}`));
    }
})();
let isInit = true;
...
const mainWatcher = watch(filePath, async () => {
  console.log(chalk.bgHex('#4B0082')(chalk.white.bold("File: 'based.js' Aggiornato")))
});
